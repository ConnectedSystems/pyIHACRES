...
def convert_taus(tau: float, v: float) -> Tuple[float, float]:
    """
    ...
    """
    alpha: float = exp(-1.0 / tau)
    beta: float = v * (1.0 - alpha)

    return (alpha, beta)


def calc_stores(tau: float, prev_store: float, v: float, e_rainfall: float) -> float:
    """Calculate flow stores."""
    (alpha, beta) = convert_taus(tau, v)
    store = (beta * e_rainfall) + alpha * prev_store

    return np.maximum(0.0, store)


# redo to allow `n` stores
def calc_flows(prev_quick: float, prev_slow: float, v_s: float,
               e_rainfall: float, area: float, tau_q: float, tau_s: float) -> Tuple[float, float, float]:
    ...
    """
    v_q: float = 1.0 - v_s  # proportional quick flow
    areal_rainfall: float = e_rainfall * area
    quick: float = calc_stores(tau_q, prev_quick, v_q, areal_rainfall)
    slow: float = calc_stores(tau_s, prev_slow, v_s, areal_rainfall)
    outflow: float = (quick + slow)

    return (quick, slow, outflow)


def routing(gw_vol: float, storage_coef: float, inflow: float, flow: float,
              irrig_ext: float, gw_exchange: float = 0.0) -> Tuple[float, float]:
    ...
    """
    tmp_gw_store: float = gw_vol + (inflow + flow + gw_exchange) - irrig_ext
    gw_store: float  # new groundwater storage volume
    c1: float
    outflow: float
    if (tmp_gw_store > 0.0):
...
def calc_outflow(flow: float, extractions: float) -> float:
    ...
    """
    return np.maximum(0.0, flow - extractions)


def calc_ft_flows(prev_quick: float, prev_slow: float, e_rain: float, 
                  recharge: float, area: float, a: float, b: float) -> Tuple[float, float, float]:
    ...
    """
    tmp_calc: float = max(0.0, prev_quick + (e_rain * area))
    if (tmp_calc > 0.0):
    ...
    assert outflow >= 0.0, f"Calculating quick store: Outflow cannot be negative: {outflow}"
    ...
    assert outflow >= 0.0, f"Calculating slow store: Outflow cannot be negative: {outflow}"

    return (quick_store, slow_store, outflow)


def calc_ft_level(outflow: float, level_params: np.ndarray) -> float:
    ...
    assert level >= 0.0, f"Stream level cannot be below 0.0, got {level}"
