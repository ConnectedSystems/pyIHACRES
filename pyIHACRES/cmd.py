...
from math import atan, exp, pi, tan, pow, log
...
def calc_cmd(prev_cmd: float, rainfall: float, et: float, 
    ...
    """
    cmd: float = prev_cmd + et + effective_rainfall + recharge - rainfall  # units in mm

    return np.maximum(0.0, cmd)


def calc_linear_interim_cmd(cmd: float, param_d: float, rainfall: float) -> float: 
    """
...
def calc_ft_interim_cmd(cmd: float, rain: float, d: float, d2: float, alpha: float) -> Tuple[float]:
    """
